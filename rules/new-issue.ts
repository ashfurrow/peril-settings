import { schedule, danger, markdown, warn } from "danger"
...
export const markRepoAsStale = wrap(
  ...
  async () => {
    ...
    warn(`result.data.pushed_at: ${result.data.pushed_at}`)
    warn(`Date.parse(result.data.pushed_at): ${Date.parse(result.data.pushed_at)}`)
    warn(`sixMonthsAgo: ${sixMonthsAgo}`)
